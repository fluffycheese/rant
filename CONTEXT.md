...
**Setup Route**
A one-time API endpoint (`/api/auth/setup`) that creates the first admin user. Only available when zero users exist in the Data Store. Once an admin exists, the endpoint returns 403. Replaces the previous CLI-based admin creation script.
